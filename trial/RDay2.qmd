...
::: {.incremental}
- Packages and package dependencies Management:wave:
- Data Importation :arrow_down:
- Break :clock9:
- Introduction to `tidyverse` :heavy_check_mark:
- Data Exploration :clipboard:
- Modelling Analysis
:::
...
- Other packages that are already installed need to be loaded explicitly.
:::
---

## What is R Repositories?
...
1. **Installing R Packages From CRAN:** For installing R Package from CRAN we need the name of the package and use the following command:
...
2. **Installing BiocManager Packages:** For installing Bioconductor packages, you should use the BiocManager package to install and manage Bioconductor packages.
...
3. **Install an R packages from GitHub**

- First, you need to install `devtools` or `remotes` packages by running the following code:
...
- Once either `devtools` or `remotes` is installed, you can use the `install_github()` function to install an R package from GitHub. The syntax is:

```{r, eval=FALSE}
devtools::install_github("github_username/github_repo")# devtools usage
remotes::install_github("github_username/github_repo")# remotes 
```
...
.libPaths()#get library location in your system
installed.packages()
```
...
- When a R package is installed, we are ready to use its functionalities.
- If we just need a sporadic use of a few functions or data inside a package we can access them with the following notation.
...
- Both methods accomplish the same thing, so it’s mostly a matter of personal preference.
---
...
![Figure from Wickham and Grolemund (2017)](images/analysisworkflow.png)
---
...
- Since each package came with data, we might use it for analysis. For example `gapminder` data came with `readr`.
:::
---
...
::: {.incremental}
- The gapminder dataset (Bryan 2017) is an excerpt from the gapminder project and contains the life expectancy at birth for 142 countries at 5 year intervals between 1952 and 2007.
- It also contains the population and the Gross Domestic Product (GDP) per Inhabitant.
- We will make some analysis and built a visualization on it later on.
:::
---
...
::: {.incremental}
- The main folder where R starts is called the working directory. - To find out, what our current working directory is, we execute the function `getwd()` for get working directory.
- RStudio projects set this working directory automatically, which is very convenient. 
- It makes it easier for us to share code and projects, by simply copying the whole folder.
- Our file paths need to be relative, not absolute. An absolute file path starts at the root of our operating system, 
:::
---

## Wranglind data with `dplyr`

::: {.incremental}
- There a are a number of ways in which we can manipulate data
-  This is sometimes referred to as `data wrangling` and within the tidyverse, this is a job for the `dplyr` package.
- `dplyr` provides functions for various operations on our data.
- Theses functions are sometimes also called __dplyr verbs__.
- All of them take a `tibble` or `data.frame` as input and always return a `tibble`. 
...
- `select`, `filter`, `mutate`, `group_by`, `summarize`,`arrange`,and others
